...
def net_train(hidden_number,EPOCH,STOP_ERROR,ALPHA,BETA1,BETA2,EPS,ETA,WEIGHT_RATE):
    ...
    for i in range(0,epoch):
        ...
        print(np.floor(i/(epoch/100)),"%",end = '')
        print("\r",end='')
        nn.zerograds()
        loss = nn(x,y)
        loss.backward()
        ...
        if i==MiddleEpoch:
            fn = file_name+"_epoch_"+str(MiddleEpoch)
            serializers.save_npz(fn, nn)
        if i==MiddleEpoch or i==Epoch-1:
            out_error,output_data = net_test(hidden_number) 
            plt.figure(2)
            ...
            plt.savefig(str(i)+'_epoch_TrainWeight_LeftOutput.pdf',bbox_inches='tight')
            ...
            plt.savefig(str(i)+'_eopch_TrainWeight_rightOutput.pdf',bbox_inches='tight')
...
for i in hidden_num:
    ...
    loss_list_train = net_train(i,Epoch,Stop_Error,Alpha,Beta1,Beta2,Eps,Eta,Weight_Decay_Rate)
    plt.figure(1)
    fig = plt.figure(figsize=(10,10))
    ax = fig.add_subplot(111)
    ax.scatter(range(0,len(loss_list_train)),loss_list_train)
    #ax.plot(loss_list_train[1:])
    ax.plot(loss_list_train)
    plt.xlabel("Epoch")
    plt.ylabel("loss")
    plt.savefig(str(i)+'loss_epoch.pdf',bbox_inches='tight')

    f_loss_list = open('hidden'+str(i)+'loss_list.csv','w',encoding='utf-8')
    csv_writer = csv.writer(f_loss_list)       
    csv_writer.writerow(loss_list_train)        
    f_loss_list.close()
