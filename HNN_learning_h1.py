...
history_data_step_num = 10
history_data_num = 2
history_data_list = []
history_data_list.append(history_data_step_num)
history_data_list.append(history_data_num)
f0 = open('history_data_step_num.csv','w',encoding='utf-8')
csv_writer0 = csv.writer(f0)       
csv_writer0.writerow(history_data_list)        
f0.close()

path1 = "chainer_data_in.csv"
path2 = "chainer_motor_out.csv"

data_in_o = pd.read_csv(path1,header=None)
data_in_o = data_in_o.values
data_out_o = pd.read_csv(path2,header=None)
data_out_o = data_out_o.values

data_in = np.zeros((data_in_o.shape[0]-history_data_step_num,data_in_o.shape[1]*history_data_num))
data_out = np.zeros((data_out_o.shape[0]-history_data_step_num,data_out_o.shape[1]))
for hi in range(history_data_step_num,data_in_o.shape[0]):
    data_in[hi-history_data_step_num,:] = np.hstack((data_in_o[hi][:],data_in_o[hi-history_data_step_num][:]))
    data_out[hi-history_data_step_num,:] = data_out_o[hi][:]

print("History data in shape : ",data_in.shape)
print("History data out shape : ",data_out.shape)   #dataのサイズを出力してcheckしてください  
...
data_in_list.append(data_in_max)
#data_mean = data_in.mean()
#data_stand = data_in.std()
#data_in_list.append(data_mean)
#data_in_list.append(data_stand)
...
data_out_list.append(data_out_max)
#data_mean = data_out.mean()
#data_stand = data_out.std()
#data_out_list.append(data_mean)
#data_out_list.append(data_stand)
...
"""      
f = open('Input_data_mean_stand.csv','w',encoding='utf-8')
csv_writer = csv.writer(f)       
csv_writer.writerow(data_in_list)        
f.close()
f2 = open('Output_data_mean_stand.csv','w',encoding='utf-8')
csv_writer2 = csv.writer(f2)
csv_writer2.writerow(data_out_list)      
f2.close()
"""
...
"""
with open('Input_data_mean_stand.csv','r') as f:
    reader = csv.reader(f)
    d = list(reader)
    d_in_ms = d[0]   #is list
    
with open('Output_data_mean_stand.csv','r') as f2:
    reader2 = csv.reader(f2)
    d = list(reader2)
    d_out_ms = d[0]  #is list
     
Data_in_ms = np.zeros((1,len(d_in_ms)))
for i in range(0,len(d_in_ms)):
    Data_in_ms[0,i] = d_in_ms[i]
Data_out_ms = np.zeros((1,len(d_out_ms)))
for i in range(0,len(d_out_ms)):
    Data_out_ms[0,i] = d_out_ms[i]
"""
...
prediction_data_out = data_out
for i in range(0,prediction_data_in.shape[1]):
    for v in range(0,prediction_data_in.shape[0]):
        #prediction_data_in[v,i] = (prediction_data_in[v,i]-Data_in_ms[0,0])/Data_in_ms[0,1]
        prediction_data_in[v,i] = prediction_data_in[v,i]/Data_in_max
...
def net_test(HiddenNumber):
    ...
    for i in range(0,prediction_data_in.shape[0]):
        ...
        for k in range(0,len(yy)):
            ...
            #y_out[i,k] = y_out[i,k] * Data_out_ms[0,1] + Data_out_ms[0,0]
            y_out[i,k] = y_out[i,k] * Data_out_max
