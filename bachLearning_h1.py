...
#print(data_in.shape)
#print(data_out.shape)   #dataのサイズを出力してcheckしてください
...
def net_train(batch_size,hidden_number,EPOCH,STOP_ERROR,ALPHA,BETA1,BETA2,EPS,ETA,WEIGHT_RATE):
    ...
    for i in range(0,epoch):
        sffindx = np.random.permutation(data_number)
        for j in range(0,data_number,batch_size):
            x_batch = x[sffindx[j:(j+batch_size) if (j+batch_size) < data_number else data_number]]
            y_batch = y[sffindx[j:(j+batch_size) if (j+batch_size) < data_number else data_number]]
            nn.zerograds()
            loss = nn(x_batch,y_batch)
            loss.backward()
            optimizer.update()
        ...
        ll = np.mean(loss_list)
        loss_list.clear()
        epoch_loss_list.append(ll)
    
        if len(epoch_loss_list) > 2:
            if np.abs(epoch_loss_list[-1] - epoch_loss_list[-2]) < loss_limit:
    ...
    #plt.plot(range(0,len(epoch_loss_list)),epoch_loss_list)
    return np.abs(ll)
...
j = 0
for i in hidden_num:
    hidden_num_loss_train[0,j] = net_train(BatchSize,i,Epoch,Stop_Error,Alpha,Beta1,Beta2,Eps,Eta,Weight_Decay_Rate)
...
plt.savefig(folder+'/'+'TrainError_Hidden.pdf',bbox_inches='tight')
#plt.show()
...
plt.savefig(folder+'/'+'TrainWeight_LeftOutput.pdf',bbox_inches='tight')
#plt.show()
...
plt.savefig(folder+'/'+'TrainWeight_rightOutput.pdf',bbox_inches='tight')
#plt.show()
...
plt.savefig(folder+'/'+'TestError_Hidden.pdf',bbox_inches='tight')
#plt.show()
...
plt.savefig(folder+'/'+'TestWeight_leftOutput.pdf',bbox_inches='tight')
#plt.show()
